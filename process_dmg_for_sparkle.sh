...
# Extract semantic version from DMG if possible
...
if [[ -d "$DMG_PATH" ]]; then
    ...
    if [[ -n "$MOUNTED_APPS" ]]; then
        INFO_PLIST="$MOUNTED_APPS/Contents/Info.plist"
        if [[ -f "$INFO_PLIST" ]]; then
            EXTRACTED_VERSION=$(defaults read "$INFO_PLIST" CFBundleShortVersionString 2>/dev/null)
            if [[ -n "$EXTRACTED_VERSION" ]]; then
                SEMANTIC_VERSION="$EXTRACTED_VERSION"
                echo "📦 Using semantic version from app bundle: $SEMANTIC_VERSION"
            fi
        fi
    fi
fi
...
if [[ ! "$CLEAN_SIGNATURE" =~ ^[A-Za-z0-9+/=]+$ ]]; then
    echo "❌ Error: Invalid signature format: $CLEAN_SIGNATURE"
    exit 1
fi

echo "🔑 Using cleaned signature: $CLEAN_SIGNATURE"
